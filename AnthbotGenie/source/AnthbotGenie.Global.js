...
// Сколько способов канонизации пути перебирать при подписи публикации в топик.
var ANTHBOT_TOPIC_MODES = 3;
 ...
 * поэтому предусмотрено три варианта подписи; рабочий определяется по факту (на curl и на живом
 * облаке подтверждён нулевой).
 *
 * @param {Object} session
 * @param {string} topic
 * @param {Object} payload
 * @param {number} [mode] 0 — закодированный путь с повторным кодированием в подписи (по умолчанию),
 *                        1 — закодированный путь без повторного кодирования,
 *                        2 — сырой путь топика
 * @returns {Object} { ok }
 */
...
/**
 * Идентификаторы зон, которые косилка косит прямо сейчас.
 * @param {Object} reported
 * @returns {number[]}
 */
