...
describe('AnthbotGenie — команды косилке', () => {
  ...
  it('после отказа нулевого варианта сразу пробуется второй, а не первый', (ctx) => {
    // Живой AWS: вариант 0 — 200, вариант 2 — 200, вариант 1 — 403. Пробовать первый раньше
    // второго значит терять круг с ожиданием сети на каждой команде. Порядок проверяется счётом
    // запросов с закодированным путём: варианты 0 и 1 дают одинаковый URL и различимы только так.
    ctx.http.mock.on((req) => req.method === 'POST' && req.url.indexOf('/topics/') >= 0 &&
      req.url.indexOf('%24aws') >= 0, { status: 403, body: 'SignatureDoesNotMatch' });
    ctx.http.mock.onPost(/\/topics\//, { status: 200, body: '{}' });
    const { mower, variables, options } = startDocked(ctx);
    ...
    // Третья означала бы, что между ними влез вариант 1.
