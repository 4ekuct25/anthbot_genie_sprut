...
> и без них код укладывается в предел (≈39 и ≈36 КБ). Вырезать их нужно по AST (например `acorn`),
> а не регулярками: в коде есть регулярные литералы вида `/^https?:\/\//i`, на которых
